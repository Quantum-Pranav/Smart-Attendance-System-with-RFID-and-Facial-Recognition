...
1.  **Libraries:** Ensure you have installed all the required libraries mentioned at the top of each `.ino` file (e.g., `UniversalTelegramBot`, `MFRC522`, `LiquidCrystal_I2C`, etc.).
2.  **Edge Impulse Model:** This code is designed to work with an Edge Impulse model. You will need to train your own model and replace the one included in the ESP32-CAM sketch.
3.  **Credentials:** Open both sketches and fill in your details in the "USER CONFIGURATION" section:
...
4.  **Wiring:** Connect the two ESP32s via their serial pins (TX/RX) and wire the peripherals (RFID, LCD) to the main ESP32 as defined in the code.
5.  **Upload:** Flash each sketch to its corresponding board.
